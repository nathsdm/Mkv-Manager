...
if ($folders.Count -gt 0) {
    ...
    if ($selectedFolder) {
        ...
        if ($firstMkvFile) {
            # Construct the command to list subtitle tracks
            ...
            $subtitleTracks = $result | ConvertFrom-Json
            $subtitleTracks = $subtitleTracks.tracks
            $tracks = @()
            $id_list = @()

            # Display subtitle tracks and build the $id_list
            ...
            $chosenTrack = $tracks | Out-GridView -Title "Select a subtitle track" -OutputMode Single
            ...
            foreach ($mkvfile in $mkvFiles) {
                Write-Host "Setting subtitle track with ID $chosenTrackId as default"
            ...
            # Success message
            Write-Host "Subtitle track with ID $chosenTrackId set as default for all MKV files in the folder: $parentFolderName"
        }
        else {
